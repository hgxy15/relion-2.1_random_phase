...
(which in all tested cases is perfectly fine), then you could use an box-size of ~"  << sLowBoxLim << "." << std::endl;
	...
	if (random_phase_switch)
	{
		//See if this is a mloptimisermpi object.
		//if (this->set_mpi())
		if (true)
		{
		        //std::cout << "\033[0;31mDEBUG ONLY! Now entering initialization for refine_mpi\033[0m"  << std::endl;
			myrandom.initialize(this, true); 
		}
		else
		{
		        //std::cout << "\033[0;33mDEBUG ONLY! Now entering initialization for refine\033[0m"  << std::endl;
			myrandom.initialize(this, false);
		}
		//myrandom.print_iter_info(this);
		std::cout << "The initialization of random phase is complete!" << std::endl;
	}
...
void MlOptimiserMpi::iterate()
{
	...
	for (iter = iter + 1; iter <= nr_iter; iter++)
    {
		...
		if (random_phase_switch)
		{
			if (this->set_mpi())
			{
				std::cout << "\033[0;31mNow entering the random phase initialization for mpi process at iteration " << iter << "\033[0m\n";
				myrandom.iterate_initialize(this,true);
			}
			else
			{
				std::cout << "\033[0;31mNow entering the random phase initialization for non mpi process at iteration " << iter << "\033[0m\n";
				myrandom.iterate_initialize(this,false);
			}
		...
		for (subset = subset_start; subset <= nr_subsets; subset++)
		{
			...
			if (verb > 0 && node->isMaster())
			{
				std::cout << "\033[0;33mWe are past the MPI_BARRIER......."  << "\033[0m\n";
			}
