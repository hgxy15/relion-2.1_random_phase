...
void autoMask(MultidimArray<RFLOAT> &img_in, MultidimArray<RFLOAT> &msk_out,
		RFLOAT ini_mask_density_threshold, RFLOAT extend_ini_mask, RFLOAT width_soft_mask_edge, RFLOAT width_exponential_edge, bool verb)
	...
	if (width_soft_mask_edge > 0.)
	{
		...
		FOR_ALL_ELEMENTS_IN_ARRAY3D(msk_cp)
		{
			...
			if (A3D_ELEM(msk_cp, k, i, j) < 0.001)
			{
				...
				if (min_r2 < width_soft_mask_edge2)
				{
					A3D_ELEM(msk_out, k, i, j) = (0.5 + 0.5 * cos( PI * sqrt(min_r2) / width_soft_mask_edge)) * exp(-1 * sqrt(min_r2) / width_soft_mask_edge);
				}
			}
			if (verb)
			{
